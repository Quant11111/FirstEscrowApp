...
function Appescrow() {
  ...
  async function getBalancePayment(){
    if(typeof window.ethereum !== 'undefined') {
      const accounts = await window.ethereum.request({method:'eth_requestAccounts'}); //peut etre inutile
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      ...
      <div className="container">
        ...
        <div className="wallet__flex">
          ...
          <div className="Buyer">
            ...
            <input type="text" placeholder="Addresse du client" onChange={changeBuyer} />
            <button onClick={getBalancePayment}>Afficher le montant verrouillé</button> 
            <button onClick={sinapsConfirmDeliver}>SinapsConfirmDeliver</button> 
          </div>

          <div className="Prestataire">
            <h3>Verrouillez votre paiement :</h3>
            <input type="text" placeholder="Addresse du prestataire" onChange={changePrestataire} />
            <input type="text" placeholder="Montant en Ethers" onChange={changeAmountEscrow} />
